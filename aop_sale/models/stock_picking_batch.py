...
class StockPickingBatch(models.Model):
    ...
    def check_loading_plan(self):
        ...
        if len(set(to_location_ids)) != 1 if to_location_ids else False:
            raise ValueError(_('You must select same location !'))

        location_dest_ids = self.picking_ids.mapped('location_dest_id').ids

        # 验证 选择的任务
        if len(set(location_dest_ids)) != 1 if location_dest_ids else False:
            ValueError(_('You must select same to location !'))

        if set(to_location_ids) - set(location_dest_ids):
            raise ValueError(_('picking location_dest_id != loading plan to_location_id'))
    ...
    def _compute_allow_partner_ids(self):
        ...
        for line in self:
            ...
            # 搜索客户合同
            start_date = picking_ids.mapped('scheduled_date')
            start_date = sorted(start_date)

            customer_contract_ids = self.env['customer.aop.contract'].search([
