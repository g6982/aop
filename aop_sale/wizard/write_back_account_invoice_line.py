...
class WriteBackAccountInvoiceLine(models.TransientModel):
    ...
    def find_customer_carrier_id(self, contract_ids, order_line_id, no_filter=False):
    ...
    def _get_customer_carrier_id(self, contract_ids, order_line_id):
        ...
        latest_carrier_id = self.find_customer_carrier_id(contract_ids, order_line_id)

        # 如果找不到，就不要条件去找
        if not latest_carrier_id:
            latest_carrier_id = self.find_customer_carrier_id(contract_ids, order_line_id, no_filter=True)
